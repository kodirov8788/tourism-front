    ...
    const formRequest = (e) => {
        ...
        if (data) {
            const postData = async () => {
                await axios.post("client/create", { name: userData.name, lastname: userData.lastname, address: userData.address, number: userData.number, trip: data })
                    .then(res => {
                        console.log(res)
                        navigate("/")
    ...
    return (
        <div className='mt-[55px] pt-[20px] flex flex-col items-center'>
            ...
            <form className='flex flex-col border w-4/5' action="" onSubmit={formRequest}>
                ...
                <label className={style.label} htmlFor="lastname">Familiya</label>
                <input required className={style.input} type="text" name='lastname' onChange={(e) => setUserData({ ...userData, lastname: e.target.value })} />
